...
const manageReceiveService = async (req, res) => {
    ...
    const supplierQuery = `SELECT * FROM skytobi."Supplier" WHERE id = $1 AND status = 'ACTIVE'`;
    ...
    const branchQuery = `SELECT * FROM skytobi."Branch" WHERE id = $1 AND status = 'ACTIVE'`;
    ...
    const referenceExistsQuery = `SELECT COUNT(*) FROM skytobi."Service" WHERE reference = $1`;
    ...
    try {
        for (let i = 0; i < rowsize; i++) {
            ...
            // Insert new service
            const insertQuery = `
                INSERT INTO skytobi."Service" (supplier, servicetype, description, otherdetails, amount, amountfrom, amountto, 
        ...
        const organisationData = await pg.query('SELECT * FROM skytobi."Organisationsettings"');
        ...
        if (req.body.reference) {
            await pg.query(
                `DELETE FROM skytobi."Service" WHERE reference = $1`,
                [req.body.reference]
            );
        } 
