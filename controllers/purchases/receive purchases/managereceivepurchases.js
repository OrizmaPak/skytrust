...
const manageReceivePurchases = async (req, res) => {
    ...
    try { 
        ...
        const supplierQuery = await pg.query(`SELECT * FROM skytobi."Supplier" WHERE id = $1`, [req.body[`supplier`]]);
        ...
        for (let i = 1; i <= rowsize; i++) {
            ...
            const inventory = await pg.query(`SELECT * FROM skytobi."Inventory" WHERE itemid = $1 AND status = 'ACTIVE'`, [itemid]);
            ...
            const branchQuery = await pg.query(`SELECT * FROM skytobi."Branch" WHERE id = $1`, [req.body[`branch`]]);
        ...
        try {
            ...
            for (const item of inventoryItems) {
                await pg.query(`INSERT INTO skytobi."Inventory" (
                ...
                const { rows: department } = await pg.query(`SELECT department FROM skytobi."Department" WHERE id = $1`, [item.department]);
                // Get the branch from the branch table
                const { rows: branch } = await pg.query(`SELECT branch FROM skytobi."Branch" WHERE id = $1`, [item.branch]);
                // Log activity for opening stock
            ...
            const orgSettings = (await pg.query(`SELECT * FROM skytobi."Organisationsettings" LIMIT 1`)).rows[0];
            ...
            if (reference) {
                try {
                    await pg.query(
                        `DELETE FROM skytobi."Inventory" WHERE reference = $1`,
                        [reference]
                    );
                } catch (error) {
