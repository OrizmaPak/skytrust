    ...
    try {
        const query = `
            SELECT 
                ...
                END AS membervalues
            FROM 
                skytobi."savingsproduct" sp
            LEFT JOIN 
                skytobi."Deduction" d ON sp.id = d.savingsproductid AND d.status = 'ACTIVE' 
            LEFT JOIN 
                skytobi."Interest" i ON sp.id = i.savingsproductid AND i.status = 'ACTIVE'
            WHERE 
                sp.status = 'ACTIVE'
            GROUP BY 
                sp.id
