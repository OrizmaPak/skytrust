...
const manageSavingsProduct = async (req, res) => {
    ...
    if (membership) {
        ...
        const queryText = `SELECT id FROM skytobi."DefineMember" WHERE id = ANY($1::int[])`;
    ...
    try {
        ...
        if (id) {
            ...
            const { rows: existingProductById } = await pg.query(`SELECT * FROM skytobi."savingsproduct" WHERE id = $1`, [id]);
            ...
            await pg.query(
                `UPDATE skytobi."savingsproduct" SET
                    productname = $1,
                    currency = $2,
                    maxbalance = $3,
                    allowdeposit = $4,
            ...
            await pg.query(`DELETE FROM skytobi."Interest" WHERE savingsproductid = $1`, [id]);
            await pg.query(`DELETE FROM skytobi."Deduction" WHERE savingsproductid = $1`, [id]);

            console.log("Inserting new interests");
            // Insert new interests
            for (const interest of interests) {
                await pg.query(
                    `INSERT INTO skytobi."Interest" (
                        savingsproductid,
                        interestname,
                        interestmethod,
            ...
            // Create new product
            const { rows: existingProduct } = await pg.query(`SELECT * FROM skytobi."savingsproduct" WHERE productname = $1`, [productname]);
            ...
            const insertProductQuery = `
                INSERT INTO skytobi."savingsproduct" (
                    productname,
                    currency,
                    maxbalance,
                    allowdeposit,
