...
const getLoanProducts = async (req, res) => {
  ...
  try {
            ...
            WHEN lp.membership IS NOT NULL THEN
              CASE 
                WHEN lp.membership ~ '^[0-9]+$' THEN
                  (SELECT dm.member FROM skytobi."DefineMember" dm WHERE dm.id = lp.membership::int)
                ELSE
    ...
    if (penaltyIds.length > 0) {
      const penaltyQuery = {
        text: `SELECT * FROM skytobi."loanfee" WHERE id = ANY($1::int[])`,
        values: [penaltyIds],
      };
