...
const manageLoanProduct = async (req, res) => {
        ...
        if (!['LOAN', 'SAVINGS'].includes(eligibilityproductcategory)) {
            ...
            if (eligibilityproductcategory === 'SAVINGS' && eligibilityproduct > 0) {
                const savingsProductQuery = {
                    text: 'SELECT * FROM skytobi."savingsproduct" WHERE id = $1',
            ...
            if (eligibilityproductcategory === 'LOAN' && eligibilityproduct > 0) {
                const loanProductQuery = {
                    text: 'SELECT * FROM skytobi."loanproduct" WHERE id = $1',
                    values: [eligibilityproduct]
                };
    ...
    if (!id) {
        const productNameQuery = {
            text: 'SELECT * FROM skytobi."loanproduct" WHERE productname = $1',
            values: [productname]
        };
    ...
    try {
        ...
        if (defaultpenaltyid) {
            const penaltyQuery = {
                text: 'SELECT * FROM skytobi."loanfee" WHERE id = $1 AND feemethod = $2',
        ...
        if (id) {
            ...
            const updateLoanProductQuery = {
                text: `UPDATE skytobi."loanproduct" SET 
            ...
            const createLoanProductQuery = {
                text: `INSERT INTO skytobi."loanproduct" (
                        productname, 
                        description, 
                        interestmethod, 
                        interestrate, 
