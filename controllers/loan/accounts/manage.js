...
const manageLoanAccount = async (req, res) => {
    ...
    try {
        ...
        const userQuery = {
            text: 'SELECT * FROM skytobi."User" WHERE id = $1',
            values: [userid]
        };
        ...
        const loanProductQuery = {
            text: 'SELECT * FROM skytobi."loanproduct" WHERE id = $1 AND ($2::text = ANY(string_to_array(membership, \'||\')) OR membership = $2::text)',
        ...
        const branchQuery = {
            text: 'SELECT * FROM skytobi."Branch" WHERE id = $1',
            values: [branch]
        };
        ...
        if(!accountnumber){
            ...
            const userAccountsQuery = {
                text: 'SELECT COUNT(*) FROM skytobi."loanaccounts" WHERE userid = $1 AND loanproduct = $2 AND member = $3 AND (dateclosed IS NULL)',
        ...
        const accountRowsQuery = `SELECT accountnumber FROM skytobi."loanaccounts" WHERE accountnumber::text LIKE $1 ORDER BY accountnumber DESC LIMIT 1`;
        ...
        if (!accountnumber) {
            ...
            if (loanProduct.eligibilityproductcategory == 'LOAN') {
                ...
                const loanAccountQuery = {
                    text: 'SELECT * FROM skytobi."loanaccounts" WHERE userid = $1 AND loanproduct = $2 AND member = $3',
                    values: [userid, loanProduct.eligibilityproduct, member]
                };
        ...
        if(accountnumber){
            const deleteScheduleQuery = {
                text: `DELETE FROM skytobi."loanpaymentschedule" WHERE accountnumber = $1`,
                values: [accountnumber]
            };
        ...
        if (loanProduct.repaymentsettings === 'ACCOUNT') {
            ...
            for (const schedule of repaymentSchedule) {
                const insertScheduleQuery = {
                    text: `INSERT INTO skytobi."loanpaymentschedule" (accountnumber, scheduledpaymentdate, scheduleamount, interestamount, status, createdby, dateadded) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        ...
        if (accountnumber) {
            ...
            const accountNumberExistsQuery = `SELECT * FROM skytobi."loanaccounts" WHERE accountnumber = $1`;
            ...
            const updateaccountnumberQuery = {
                text: `UPDATE skytobi."loanaccounts" SET 
            ...
            const createaccountnumberQuery = {
                text: `INSERT INTO skytobi."loanaccounts" 
        ...
        const installmentsQuery = {
            text: `SELECT * FROM skytobi."loanpaymentschedule" WHERE accountnumber = $1`,
