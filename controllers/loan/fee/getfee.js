...
const getLoanFees = async (req, res) => {
    try {
        let query = {
            text: `SELECT lf.*, CONCAT(acc.groupname, ' - ', acc.accounttype) AS glaccountname FROM skytobi."loanfee" lf
                   LEFT JOIN skytobi."Accounts" acc ON lf.glaccount = acc.accountnumber`,
            values: []
        };
        ...
        const countQuery = {
            text: `SELECT COUNT(*) FROM skytobi."loanfee" lf ${whereClause}`,
