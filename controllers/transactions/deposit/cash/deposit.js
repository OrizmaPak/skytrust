...
const processCashCollection = async (req, res) => {
    ...
    try {
        ...
        const { rows: cashierLimitData } = await pg.query(`
            SELECT depositlimit FROM skytobi."Cashierlimit" WHERE cashier = $1 AND status = 'ACTIVE'
        `, [userid]);
        ...
        const { rows: orgSettingsData } = await pg.query(`
            SELECT default_cash_account FROM skytobi."Organisationsettings" WHERE status = 'ACTIVE'
        `);
        ...
        for (let i = 1; i <= rowsize; i++) {
            ...
            const defaultCashTransactionQuery = `
                INSERT INTO skytobi."banktransaction" 
