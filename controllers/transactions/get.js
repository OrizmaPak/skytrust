...
const getTransactions = async (req, res) => {
    ...
    try {
        let query = {
            text: `SELECT * FROM skytobi."transaction"`,
            values: []
        };
        ...
        for (let transaction of transactions) {
            ...
            if (whichaccount === 'PERSONAL') {
                ...
                if (savings.length > 0) {
                    ...
                    const { rows: users } = await pg.query(`SELECT firstname, lastname, othernames FROM skytobi."User" WHERE id = $1`, [userid]);
                    if (users.length > 0) {
                        const { firstname, lastname, othernames } = users[0];
                        accountName = `${firstname} ${lastname} ${othernames}`.trim();
                    }
                ...
                const { rows: propertyAccounts } = await pg.query(`SELECT productid FROM skytobi."propertyaccount" WHERE accountnumber = $1`, [accountnumber]);
                if (propertyAccounts.length > 0) {
                    const { userid } = propertyAccounts[0];
                    const { rows: theuser } = await pg.query(`SELECT firstname, lastname, othernames FROM skytobi."User" WHERE id = $1`, [userid]);
                    if (theuser.length > 0) {
                ...
                const { rows: rotaryAccounts } = await pg.query(`SELECT userid FROM skytobi."rotaryaccount" WHERE accountnumber = $1`, [accountnumber]);
                ...
                const { rows: glAccounts } = await pg.query(`SELECT accountnumber FROM skytobi."Accounts" WHERE accountnumber = $1`, [accountnumber]);
        ...
        const countQuery = {
            text: `SELECT COUNT(*) FROM skytobi."transaction" ${whereClause}`,
