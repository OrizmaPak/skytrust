...
const processWithdrawal = async (req, res) => {
    ...
    try {
        ...
        const { rows: branchData } = await pg.query(`
            SELECT * FROM skytobi."Branch" WHERE id = $1 AND status = 'ACTIVE'
        `, [branch]);
        ...
        const { rows: cashierLimitData } = await pg.query(`
            SELECT withdrawallimit FROM skytobi."Cashierlimit" WHERE cashier = $1 AND status = 'ACTIVE'
        `, [userid]);
        ...
        for (let i = 1; i <= rowsize; i++) {
            ...
            const { rows: orgSettingsData } = await pg.query(`
                SELECT default_cash_account, default_allocation_account, personal_account_prefix FROM skytobi."Organisationsettings" WHERE status = 'ACTIVE'
            `);
            ...
            if(!originalCashref &&allocation == 1){
                ...
                const { rows: userByPhoneData } = await pg.query(`
                    SELECT id FROM skytobi."User" WHERE phone = $1 AND status = 'ACTIVE'
                `, [phonenumber]);
            ...
            const bankTransactionQuery = {
                text: `INSERT INTO skytobi."banktransaction" 
