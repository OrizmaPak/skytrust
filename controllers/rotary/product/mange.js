    ...
    try {
            ...
            for (const memberId of memberIds) {
                ...
                if (trimmedMemberId) {
                    const { rows } = await pg.query({
                        text: `SELECT 1 FROM skytobi."DefineMember" WHERE id = $1`,
        ...
        if (productofficer) {
            const { rows: userRoleRows } = await pg.query({
                text: `SELECT role FROM skytobi."User" WHERE id = $1`,
                values: [user.id]
            });
        ...
        if (id) {
            ...
            const updateQuery = {
                text: `UPDATE skytobi."rotaryProduct" 
            ...
            const insertQuery = {
                text: `INSERT INTO skytobi."rotaryProduct" (product, member, useraccount, registrationcharge, createdby, productofficer, currency, description, poolnumber, rotaryschedule, frequency, frequencynumber, dateadded, status, registrationpoint) 
