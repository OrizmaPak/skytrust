...
const getTransactionsAndBalance = async (req, res) => {
  ...
  try {
    ...
    // -----------------------------------------------------------------------
    const orgSettingsQuery = `SELECT * FROM skytobi."Organisationsettings"`;
    ...
    const notActiveQuery = `
      ...
      FROM skytobi."transaction" t
      LEFT JOIN skytobi."User" u ON t.userid = u.id
      LEFT JOIN skytobi."User" cu ON t.createdby = cu.id
