...
const rejectService = async (req, res) => {
    ...
    try {
        ...
        const serviceQuery = `SELECT * FROM skytobi."Service" WHERE id = $1`;
        ...
        let staffphone;
        if (staff) {
            const staffQuery = `SELECT * FROM skytobi."User" WHERE id = $1`;
        ...
        const { rows: [organisationSettings] } = await pg.query(`SELECT * FROM skytobi."Organisationsettings"`);
        ...
        const insertQuery = `
            INSERT INTO skytobi."Service" (serviceid, supplier, staff, servicetype, description, amount, amountfrom, amountto, servicestartdate, serviceenddate, branch, dateadded, createdby, issue, reference, status)
