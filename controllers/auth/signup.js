...
const signup = async (req, res) => {
    ...
    try {
        ...
        const { rows: theuser } = await pg.query(`SELECT * FROM skytobi."User" WHERE email = $1`, [email]);
        const { rows: phoneUser } = await pg.query(`SELECT * FROM skytobi."User" WHERE phone = $1`, [phone]);
        ...
        const { rows: [saveuser] } = await pg.query(`
        ...
        const setupResult = await autoAddMembershipAndAccounts(req, res, 0);
        if (!setupResult.status) {
            throw new Error(setupResult.error || 'Membership and account creation failed');
        }

        const { rows: [details] } = await pg.query(`SELECT * FROM skytobi."User" WHERE id = $1`, [userId]);
        ...
        return res.status(StatusCodes.OK).json({
            ...
            data: {
                ...
                setup: {
                    membershipsCreated: setupResult.membershipsCreated,
                    savingsAccountsCreated: setupResult.accountsCreated
                }
            },
            errors: []
        });
