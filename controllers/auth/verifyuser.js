...
async function verifyuser(req, res) {
    ...
    try {
        ...
        const {rows:[user]} = await pg.query(`SELECT * FROM skytobi."VerificationToken" WHERE token = $1`, [token] )
        console.log(user)
        if(user && user.token == token && user.expires > new Date()){
            await pg.query(`DELETE FROM skytobi."VerificationToken" WHERE token = $1`, [token])
            await pg.query(`UPDATE skytobi."User" SET emailverified = $1 WHERE id = $2`, [new Date(), user.id])
            const responseData = {
                error: false,
