...
const resetPassword = async (req, res) => {
  try {
    ...
    const { rows: [user] } = await pg.query(`SELECT * FROM skytobi."VerificationToken" WHERE token = $1`, [passwordtoken]);
    if (!user) {
      return res.status(StatusCodes.BAD_REQUEST).json({
        status: false,
        message: "Expired token",
    ...
    await pg.query(`DELETE FROM skytobi."VerificationToken" WHERE token = $1`, [passwordtoken]);
