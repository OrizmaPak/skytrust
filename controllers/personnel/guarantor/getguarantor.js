    ...
    try {
        let query = {
            text: `SELECT g.*, CONCAT(u.firstname, ' ', u.lastname, ' ', COALESCE(u.othernames, '')) AS personnelname 
                   FROM skytobi."guarantor" g 
                   JOIN skytobi."User" u ON g.userid = u.id`,
            values: []
        };
        ...
        const countQuery = {
            text: `SELECT COUNT(*) FROM skytobi."guarantor" g 
                   JOIN skytobi."User" u ON g.userid = u.id ${whereClause}`,
