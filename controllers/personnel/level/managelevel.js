...
const saveOrUpdateLevel = async (req, res) => {
    ...
    try {
        ...
        if (id) {
            ...
            await pg.query(`DELETE FROM sky."allowances" WHERE level = $1`, [id]);
            await pg.query(`DELETE FROM sky."deductions" WHERE level = $1`, [id]);
        }
        ...
        if (id) {
            await pg.query(
                `UPDATE sky."level" SET level = $1, description = $2, basicsalary = $3, dateadded = NOW(), createdby = $4, status = 'ACTIVE' WHERE id = $5`,
                [level, description, basicsalary, user.id, id]
            );
        } else {
            const { rows } = await pg.query(
                `INSERT INTO sky."level" (level, description, basicsalary, dateadded, createdby, status) VALUES ($1, $2, $3, NOW(), $4, 'ACTIVE') RETURNING id`,
        ...
        for (let i = 1; i <= allowancerowsize; i++) {
            ...
            await pg.query(
                `INSERT INTO sky."allowances" (level, allowance, allowancetype, dateadded, createdby, status) VALUES ($1, $2, $3, NOW(), $4, 'ACTIVE')`,
        ...
        for (let i = 1; i <= deductionrowsize; i++) {
            ...
            await pg.query(
                `INSERT INTO sky."deductions" (level, deduction, deductiontype, dateadded, createdby, status) VALUES ($1, $2, $3, NOW(), $4, 'ACTIVE')`,
