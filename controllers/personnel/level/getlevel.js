...
const getLevel = async (req, res) => {
    ...
    try {
        let query = {
            text: `SELECT * FROM skytobi."level" WHERE status = 'ACTIVE'`,
            values: []
        };
        ...
        for (let level of levels) {
            ...
            const allowancesResult = await pg.query(
                `SELECT * FROM skytobi."allowances" WHERE level = $1`,
                [levelId]
            );
            level.allowances = allowancesResult.rows;

            const deductionsResult = await pg.query(
                `SELECT * FROM skytobi."deductions" WHERE level = $1`,
                [levelId]
            );
        ...
        const countQuery = {
            text: `SELECT COUNT(*) FROM skytobi."level" ${whereClause}`,
