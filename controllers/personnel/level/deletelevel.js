...
const deleteLevel = async (req, res) => {
    ...
    try {
        ...
        const { rows: levelRows } = await pg.query(
            `SELECT * FROM skytobi."level" WHERE id = $1 AND status = 'ACTIVE'`,
            [id]
        );
        ...
        const { rows: userRows } = await pg.query(
            `SELECT * FROM skytobi."User" WHERE level = $1 AND status = 'ACTIVE'`,
            [id]
        );
        ...
        await pg.query(
            `UPDATE skytobi."level" SET status = 'DELETED' WHERE id = $1`,
            [id]
        );
