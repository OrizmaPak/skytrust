...
const saveOrUpdatePromotion = async (req, res) => {
    ...
    try {
        ...
        const { rowCount: levelCount } = await pg.query({
            text: `SELECT * FROM skytobi."level" WHERE "id" = $1`,
            values: [level]
        });
    ...
    try {
        if (id) {
            ...
            const { rowCount } = await pg.query({
                text: `UPDATE skytobi."promotiondemotion" 
            ...
            // Save new promotion
            await pg.query({
                text: `INSERT INTO skytobi."promotiondemotion" ("userid", "title", "level", "type", "dateadded", "createdby", "status") 
