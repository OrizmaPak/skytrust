    ...
    try {
        ...
        const userQuery = {
            text: `SELECT u.*, l.level as levelname
                   FROM skytobi."User" u
                   LEFT JOIN skytobi."level" l ON u.level = l.id
                   WHERE u.id = $1`,
            values: [userid]
        };
        ...
        const promises = tables.map(table => {
            ...
            if (table === 'level') {
                query = {
                    text: `SELECT l.*, 
                                  (SELECT json_agg(a) FROM skytobi."allowances" a WHERE a.level = l.id AND a.status = 'ACTIVE') as allowances,
                                  (SELECT json_agg(d) FROM skytobi."deductions" d WHERE d.level = l.id AND d.status = 'ACTIVE') as deductions
                           FROM skytobi."level" l
                           WHERE l.id = $1 AND l.status = 'ACTIVE'`,
                    values: [userData.level]
                };
            } else {
                query = {
                    text: `SELECT * FROM skytobi."${table}" WHERE userid = $1 AND status = 'ACTIVE'`,
                    values: [userid]
                };
            }
