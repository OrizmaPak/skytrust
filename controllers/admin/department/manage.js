...
const defineDepartment = async (req, res) => {
    ...
    try {
        ...
        const { rows: thedepartment } = await pg.query(
            `SELECT * FROM skytobi."Department" 
            WHERE (department = $1 AND branch = $2) 
            OR (branch = $2 AND department = $1 AND (skytobi."Department".id != $3 OR $3 = '0'))`, 
        ...
        if (id) {
            if (id && status) {
                ...
                query = await pg.query(`UPDATE skytobi."Department" SET 
            ...
            query = await pg.query(`INSERT INTO skytobi."Department" 
