...
const createbranch = async (req, res) => {
    ...
    try {
        ...
        const { rows: userExists } = await pg.query(`SELECT * FROM skytobi."User" WHERE id = $1`, [userid]);
        ...
        if (!id) { // Check if branch already exists using raw query
            const { rows: thebranch } = await pg.query(`SELECT * FROM skytobi."Branch" WHERE branch = $1`, [branch]);
        ...
        if (id) {
            if (status) {
                query = await pg.query(`UPDATE skytobi."Branch" SET 
                    status = $1,
                    lastupdated = $2
            ...
            query = await pg.query(`INSERT INTO skytobi."Branch" 
