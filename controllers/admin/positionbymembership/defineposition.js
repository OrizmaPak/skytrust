...
const definepositionbymembership = async (req, res) => {
    ...
    try {
        if (!id) {
            ...
            const { rows: themember } = await pg.query(`SELECT * FROM skytobi."DefineMember" WHERE id = $1`, [member]);
            ...
            const { rows: thepositions } = await pg.query(`SELECT * FROM skytobi."Position" WHERE member = $1`, [member]);
            ...
            if (branch) {
                const { rows: thebranch } = await pg.query(`SELECT * FROM skytobi."Branch" WHERE id = $1`, [branch]);
            ...
            if (userid) {
                const { rows: theuser } = await pg.query(`SELECT * FROM skytobi."User" WHERE id = $1`, [userid]);
                if (theuser.length == 0) {
                    return res.status(StatusCodes.BAD_REQUEST).json({
                        status: false,
        ...
        if (id) {
            if (id && status) {
                query = await pg.query(`UPDATE skytobi."Position" SET 
            ...
            query = await pg.query(`INSERT INTO skytobi."Position" 
