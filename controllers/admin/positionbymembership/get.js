  ...
  try {
    let query = {
      text: `SELECT p.*, b.branch AS branchname, m.member AS membername, 
             ...
             FROM skytobi."Position" p 
             LEFT JOIN skytobi."Branch" b ON p.branch = b.id 
             LEFT JOIN skytobi."DefineMember" m ON p.member = m.id 
             LEFT JOIN skytobi."User" u ON p.userid = u.id
    ...
    const countQuery = {
      text: `SELECT COUNT(*) FROM skytobi."Position" p WHERE 1=1 ${whereClause}`,
