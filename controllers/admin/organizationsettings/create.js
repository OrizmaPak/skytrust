  ...
  try {
    ...
    for (const account of defaultAccounts.filter(a => a)) {
      const { rowCount } = await pg.query(
        `SELECT 1 FROM skytobi."Accounts" WHERE accountnumber = $1`,
        [account]
      );
      if (rowCount === 0) {
        errors.push({
    ...
    // Database operation
    const { rows: existing } = await pg.query('SELECT * FROM skytobi."Organisationsettings"');
    ...
    if (existing.length === 0) {
      ...
      const { rows } = await pg.query(
        `INSERT INTO skytobi."Organisationsettings" (${columns}) 
