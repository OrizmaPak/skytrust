...
const managerejection = async (req, res) => {
    ...
    try {
        ...
        if (id) {
            query = `UPDATE skytobi."Rejecttransactiondate" SET rejectiondate = COALESCE($1, rejectiondate), reason = COALESCE($2, reason), status = COALESCE($3, status) WHERE id = $4`;
            params = [rejectiondate, reason, req.body.status, id];
        } else {
            // Checking if rejection date already exists
            const rejectionDateExists = await pg.query(`SELECT * FROM skytobi."Rejecttransactiondate" WHERE rejectiondate = $1`, [rejectiondate]);
            ...
            query = `INSERT INTO skytobi."Rejecttransactiondate" (rejectiondate, reason) VALUES ($1, $2)`;
