        ...
        if (role) {
            ...
            if (existingRoles.length > 0) {
                ...
                if (status && status !== 'ACTIVE') {
                    ...
                    await pg.query(`UPDATE sky."Roles" SET status = $1 WHERE role = $2`, [status, role]);
                    await pg.query(`UPDATE sky."User" SET permissions = $1 WHERE role = $2`, [status, role]);
                    ...
                    await pg.query(`UPDATE sky."Roles" SET permissions = $1, description = $2, status = $3 WHERE role = $4`, [permissions, description, status, role]);
                    await pg.query(`UPDATE sky."User" SET permissions = $1 WHERE role = $2`, [permissions, role]);
                    ...
                    await pg.query(`UPDATE sky."Roles" SET permissions = $1, description = $2 WHERE role = $3`, [permissions, description, role]);
                    await pg.query(`UPDATE sky."User" SET permissions = $1 WHERE role = $2`, [permissions, role]);
                    await activityMiddleware(req, user.id, `Role '${role}' permissions updated successfully`);
                }
            } else {
                // If the role does not exist, create it
                await pg.query(`INSERT INTO sky."Roles" (role, permissions, description, status) VALUES ($1, $2, $3, $4)`, [role, permissions, description, status || 'ACTIVE']);
