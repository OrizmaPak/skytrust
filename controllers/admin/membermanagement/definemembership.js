...
const definemembership = async (req, res) => {
    ...
    try {
        if(!id){// Check if branch already exists using raw query
            const { rows: themember } = await pg.query(`SELECT * FROM skytobi."DefineMember" WHERE member = $1`, [member]);
        ...
        if (id) {
            if(status){
                query = await pg.query(`UPDATE skytobi."DefineMember" SET 
                    status = $1,
                    lastupdated = $2,
                    addmember = $3
            ...
            query = await pg.query(`INSERT INTO skytobi."DefineMember" 
