...
const addStaffToRegistrationPoint = async (req, res) => {
    ...
    try {
        const { rows: [user] } = await pg.query(`SELECT * FROM skytobi."User" WHERE id = $1`, [id]);
        ...
        if (registrationpoint != 0) {
            ...
            await pg.query(`UPDATE skytobi."User" SET registrationpoint = $1 WHERE id = $2`, [registrationpoint, id]);
            ...
            await pg.query(`UPDATE skytobi."User" SET registrationpoint = 0 WHERE id = $1`, [id]);
