...
const manageRegistrationPoint = async (req, res) => {
    ...
    try {
        if (!id) { // Create new registration point
            ...
            const registrationPointExists = await pg.query(`SELECT * FROM skytobi."Registrationpoint" WHERE registrationpoint = $1 AND branch = $2`, [registrationpoint, branch]);
            ...
            const { rows: [newRegistrationPoint] } = await pg.query(`
                INSERT INTO skytobi."Registrationpoint" (registrationpoint, description, branch, datecreated, createdby)
            ...
            if (status && status.toLowerCase() === 'deleted') {
                const { rows: usersInRegistrationPoint } = await pg.query(`
                    SELECT * FROM skytobi."User" WHERE registrationpoint = $1
                `, [id]);
            ...
            const { rows: [updatedRegistrationPoint] } = await pg.query(`
                UPDATE skytobi."Registrationpoint"
