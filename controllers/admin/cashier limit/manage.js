...
const manageCashierLimit = async (req, res) => {
    try {
        ...
        const { rows: [userExists] } = await pg.query(`SELECT * FROM skytobi."User" WHERE id = $1`, [cashier]);
        if (!userExists) {
        ...
        const { rows: [cashierLimitExists] } = await pg.query(`SELECT * FROM skytobi."Cashierlimit" WHERE cashier = $1`, [cashier]);
        ...
        if (id) {
            ...
            if (status) {
                const { rows: [updatedCashierLimit] } = await pg.query(`UPDATE skytobi."Cashierlimit" SET status = $1 WHERE id = $2 RETURNING *`, [status, id]);
                ...
                const { rows: [updatedCashierLimit] } = await pg.query(`UPDATE skytobi."Cashierlimit" SET depositlimit = $1, withdrawallimit = $2 WHERE id = $3 RETURNING *`, [depositlimit, withdrawallimit, id]);
                if (!updatedCashierLimit) {
                    return res.status(StatusCodes.NOT_FOUND).json({
                        status: false,
                        message: "Cashier limit not found",
            ...
            const { rows: [newCashierLimit] } = await pg.query(`INSERT INTO skytobi."Cashierlimit" (cashier, depositlimit, withdrawallimit, status, dateadded, createdby) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, [cashier, depositlimit, withdrawallimit, 'ACTIVE', new Date(), req.user.id]);
