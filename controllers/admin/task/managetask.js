...
const manageTask = async (req, res) => {
    try {
        ...
        if(!id){
            const { rows: [branchExists] } = await pg.query(`SELECT * FROM skytobi."Branch" WHERE id = $1`, [branch]);
            if (!branchExists) {
    ...
    const { rows: [branchName] } = await pg.query(`SELECT branch FROM skytobi."Branch" WHERE id = $1`, [branch]);
        ...
        if (id) {
            ...
            if (status) {
                ...
                if (assignedto) {
                    ...
                    for (let user of assignedUsers) {
                        const { rows: [assignedUser] } = await pg.query(`SELECT email FROM skytobi."User" WHERE id = $1`, [user]);
                        if (assignedUser) {
                            await sendEmail({
            ...
            const { rows: [newTask] } = await pg.query(`
                INSERT INTO skytobi."Task" (title, description, priority, assignedto, branch, startdate, enddate, taskstatus, status, createdby)
            ...
            if (assignedto) {
                const assignedUsers = assignedto.split('||');
                for (let user of assignedUsers) {
                    const { rows: [assignedUser] } = await pg.query(`SELECT email FROM skytobi."User" WHERE id = $1`, [user.id]);
                    if (assignedUser) {
                        await sendEmail({
                            to: assignedUser.email,
                            subject: 'Task Assigned',
