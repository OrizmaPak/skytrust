...
const manageSubtask = async (req, res) => { 
    try {
        ...
        if (id) {
            ...
            const { rows: [subtaskExists] } = await pg.query(`SELECT * FROM skytobi."Subtask" WHERE id = $1`, [id]);
            if (!subtaskExists) {
        ...
        for (let id of assignedToIds) {
            const { rows: [user] } = await pg.query(`SELECT id FROM skytobi."User" WHERE id = $1`, [id]);
            if (!user) {
                return res.status(StatusCodes.BAD_REQUEST).json({
                    status: false,
       ...
       if(!id){ // Check if start date and end date are within task's start and end date
        ...
        // const { rows: [taskAssignedTo] } = await pg.query(`SELECT assignedto FROM skytobi."Task" WHERE id = $1`, [task]);
        ...
        if (!id) {
            const { rows: [subtaskExists] } = await pg.query(`SELECT * FROM skytobi."Subtask" WHERE task = $1 AND title = $2`, [task, title]);
            if (subtaskExists) {
