    ...
    try {
        let query = {
            text: `SELECT m.*, 
                   ...
                   LEFT JOIN skytobi."User" u ON m.userid = u.id
                   LEFT JOIN skytobi."Position" p ON m.member = p.member AND m.userid = p.userid
                   LEFT JOIN skytobi."Branch" ub ON u.branch = ub.id
                   LEFT JOIN skytobi."Branch" pb ON p.branch = pb.id`,
            values: []
        };
        ...
        const countQuery = {
            text: `SELECT COUNT(*) FROM skytobi."Membership" m ${whereClause}`,
