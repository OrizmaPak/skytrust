...
const makesales = async (req, res) => {
    ...
    if(reference){
        ...
        const { rows: existingTransactionRows } = await pg.query(`SELECT * FROM skytobi."transaction" WHERE transactionref = $1`, [reference]);
    ...
    try {
        ...
        const { rows: orgSettingsRows } = await pg.query(`SELECT default_cost_of_sales_account, default_income_account FROM skytobi."Organisationsettings" LIMIT 1`);
        ...
        const itemnames = [];
        for (let i = 0; i < itemids.length; i++) {
            const { rows: itemRows } = await pg.query(`SELECT itemname FROM skytobi."Inventory" WHERE id = $1`, [itemids[i]]);
        ...
        const { rows: branchRows } = await pg.query(`SELECT branch FROM skytobi."Branch" WHERE id = $1`, [branchfrom]);
        ...
        const salesData = {
            ...
            items: await Promise.all(itemids.map(async (itemId, index) => {
                ...
                const { rows: inventoryRows } = await pg.query(`SELECT itemname FROM skytobi."Inventory" WHERE itemid = $1`, [itemId]);
