    ...
    try {
        ...
        let inventoryQuery = {
            text: `
                ...
                JOIN skytobi."transaction" T ON I."reference" = T."transactionref"
        ...
        if (branch) {
            ...
            const branchRes = await pg.query({
                text: `
                    SELECT B.branch 
                    FROM skytobi."Branch" B
                    JOIN skytobi."User" U ON B.id = U.branch
                    WHERE U.id = $1
                `,
                values: [userid]
            });
        ...
        const transactionPromises = Object.keys(groupedInventory).map(async (reference) => {
            try {
                ...
                const transactionRes = await pg.query({
                    text: `SELECT * FROM skytobi."transaction" WHERE transactionref = $1`,
                    values: [reference]
                });
