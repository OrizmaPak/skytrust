...
const createOrUpdateAccount = async (req, res) => {
    ...
    try {
        ...
        const { rows: orgSettingsRows } = await pg.query(`SELECT * FROM skytobi."Organisationsettings" WHERE id = 1`);
        ...
        if (id) {
            ...
            const { rows: existingAccountRows } = await pg.query(`SELECT * FROM skytobi."Accounts" WHERE id = $1`, [id]);
            ...
            const { rows: accountRows } = await pg.query(`SELECT accountnumber FROM skytobi."Accounts" WHERE accountnumber LIKE '${prefix}%' ORDER BY accountnumber DESC LIMIT 1`);
            ...
            const { rows: existingAccountRows } = await pg.query(`SELECT * FROM skytobi."Accounts" WHERE accountnumber = $1`, [accountnumber]);
            if (existingAccountRows.length > 0) {
        ...
        if (id) {
            ...
            await pg.query(`UPDATE skytobi."Accounts" SET groupname = $1, description = $2, dateadded = $3, createdby = $4 WHERE id = $5`, 
                [groupname, description, new Date(), user.id, id]);
            await activityMiddleware(req, user.id, 'Account updated successfully', 'ACCOUNT');
        } else {
            // Create new account
            await pg.query(`INSERT INTO skytobi."Accounts" (accountnumber, groupname, accounttype, description, status, dateadded, createdby) VALUES ($1, $2, $3, $4, 'ACTIVE', $5, $6)`, 
