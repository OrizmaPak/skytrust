...
const updateItemStatus = async (req, res) => {
    ...
    try {
        ...
        if (key === 'deliveryrequested') {
            query = {
                text: `UPDATE skytobi."propertyitems" SET "${key}" = $1, "deliveryrequestdate" = NOW() WHERE id = $2 RETURNING *`,
                values: [value, id]
            };
        } else {
            query = {
                text: `UPDATE skytobi."propertyitems" SET "${key}" = $1 WHERE id = $2 RETURNING *`,
                values: [value, id]
            };
        }
