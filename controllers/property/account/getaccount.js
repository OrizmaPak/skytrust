  ...
  try {
    ...
    const propertyAccountQuery = `
      SELECT 
        pa.*,
        pp.* 
      FROM skytobi."propertyaccount" pa
      JOIN skytobi."propertyproduct" pp 
    ...
    for (const accountRow of accountRows) {
      ...
      const userQuery = {
        text: `SELECT firstname, lastname, othernames, branch FROM skytobi."User" WHERE id = $1`,
      ...
      const branchQuery = {
        text: `SELECT branch FROM skytobi."Branch" WHERE id = $1`,
      ...
      if (accountRow.member) {
        ...
        for (const memberId of memberIds) {
          const memberQuery = {
            text: `SELECT member FROM skytobi."DefineMember" WHERE id = $1`,
            values: [memberId]
          };
      ...
      const itemsQuery = {
        text: `SELECT * FROM skytobi."propertyitems" WHERE accountnumber = $1`,
      ...
      for (const item of itemRows) {
        const inventoryQuery = {
          text: `SELECT itemname FROM skytobi."Inventory" WHERE itemid = $1`,
          values: [item.itemid]
        };
      ...
      const installmentsQuery = {
        text: `SELECT * 
               FROM skytobi."propertyinstallments"
