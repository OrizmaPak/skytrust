...
const saveCompositeDetails = async (req, res) => {
    ...
    try {
        ...
        const { rowCount: existingCount } = await pg.query({
            text: `SELECT 1 FROM skytobi."compositedetails" WHERE compositeid = $1`,
            values: [compositeid]
        });
        ...
        if (existingCount > 0) {
            await pg.query({
                text: `DELETE FROM skytobi."compositedetails" WHERE compositeid = $1`,
                values: [compositeid]
            });
        }

        // Prepare insert query
        let insertQuery = `INSERT INTO skytobi."compositedetails" (compositeid, itemid, qty, createdby, dateadded, status) VALUES `;
        const values = [];
        let valueIndex = 1;
