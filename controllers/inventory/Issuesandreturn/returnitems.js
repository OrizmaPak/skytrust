...
const managereturnitems = async (req, res) => {
    ...
    const branchExists = await pg.query(`SELECT * FROM skytobi."Branch" WHERE id = $1`, [branch]);
    ...
    const departmentExists = await pg.query(`SELECT * FROM skytobi."Department" WHERE id = $1 AND branch = $2`, [department, branch]);
    ...
    const supplierExists = await pg.query(`SELECT * FROM skytobi."Supplier" WHERE id = $1`, [itemDetails.supplier]);
    ...
    try {
        ...
        for (let i = 0; i < rowsize; i++) {
            ...
            const itemExists = await pg.query(`SELECT * FROM skytobi."Inventory" WHERE itemid = $1 AND department = $2 AND branch = $3 AND status = 'ACTIVE'`, [itemids[i], department, branch]);
            ...
            const qtyWithIssues = await pg.query(`SELECT SUM(qty) FROM skytobi."Inventory" WHERE itemid = $1 AND department = $2 AND branch = $3 AND status = 'ACTIVE' AND transactiondesc LIKE '%Issue%'`, [itemids[i], department, branch]);
            ...
            if (reference) {
                await pg.query(`DELETE FROM skytobi."Inventory" WHERE reference = $1`, [reference]);
            }

            // Insert the data into the Inventory table
            await pg.query(`INSERT INTO skytobi."Inventory" (itemid, branch, department, qty, status, transactiondesc, dateadded, createdby, itemname, units, cost, price, pricetwo, beginbalance, minimumbalance, "group", applyto, itemclass, composite, compositeid, description, imageone, imagetwo, imagethree, sellingprice, reference, transactiondate, issue, issuetype, supplier) 
