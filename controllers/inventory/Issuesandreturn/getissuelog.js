...
const getissuelog = async (req, res) => {
    ...
    let query = `
        ...
        FROM skytobi."Inventory" AS Inventory
        JOIN skytobi."Branch" AS Branch ON Inventory.branch::int = Branch.id
        JOIN skytobi."Department" AS Department ON Inventory.department::int = Department.id
        JOIN skytobi."issue" AS Issue ON Inventory.issuetype::int = Issue.id
