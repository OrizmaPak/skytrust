...
const manageIssueType = async (req, res) => {
    ...
    try {
        if (!id) {
            const existingIssueType = await pg.query(`SELECT * FROM skytobi."issue" WHERE issuetype = $1`, [issuetype]);
            ...
            await pg.query(`INSERT INTO skytobi."issue" (issuetype, createdby, dateadded) VALUES ($1, $2, $3)`, [issuetype, user.id, new Date()]);
            ...
            if (status === "DELETED") {
                const { rows: inventoryItems } = await pg.query(`SELECT * FROM skytobi."Inventory" WHERE issuetype = $1`, [id]);
            ...
            await pg.query(
                `UPDATE skytobi."issue" SET issuetype = COALESCE($1, issuetype), status = COALESCE($2, status) WHERE id = $3`,
                [issuetype, status, id]
            );
