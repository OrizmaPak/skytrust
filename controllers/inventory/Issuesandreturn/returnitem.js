...
const updateReturnItem = async (req, res) => {
    ...
    try {
        ...
        const { rows: itemExists } = await pg.query(`SELECT * FROM skytobi."Inventory" WHERE id = $1 AND status = 'ACTIVE'`, [id]);
        ...
        if (supplier) {
            ...
            const { rows: [supplierExists] } = await pg.query(`SELECT * FROM skytobi."Supplier" WHERE id = $1`, [supplier]);
            if (!supplierExists) {
            ...
            const { rows: [staffExists] } = await pg.query(`SELECT * FROM skytobi."User" WHERE id = $1`, [staff]);
            if (!staffExists) {
