...
const getpendingrequisition = async (req, res) => {
    ...
    let queryString = `SELECT * FROM skytobi."Inventory" WHERE status = $1 AND transactiondesc LIKE '%Requisition%'`;
    ...
    try {
        ...
        for (const item of inventory) {
            ...
            if (item.qty < 0) {
                ...
                const { rows: [{ branch: branchfromname }] } = await pg.query(`SELECT branch FROM skytobi."Branch" WHERE id = $1`, [item.branch]);
                const { rows: [{ department: departmentfromname }] } = await pg.query(`SELECT department FROM skytobi."Department" WHERE id = $1`, [item.department]);
                ...
                // Fetch branch and department names
                const { rows: [{ branch: branchtoname }] } = await pg.query(`SELECT branch FROM skytobi."Branch" WHERE id = $1`, [item.branch]);
                const { rows: [{ department: departmenttoname }] } = await pg.query(`SELECT department FROM skytobi."Department" WHERE id = $1`, [item.department]);
