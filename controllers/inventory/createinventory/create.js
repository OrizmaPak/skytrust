    ...
    try {
        for (let i = 1; i <= rowsize; i++) {
            ...
            for (let br of branches) {
                ...
                // const { rows: branchExists } = await pg.query(`SELECT * FROM skytobi."Branch" WHERE id = $1`, [br]);
                ...
                const { rows: itemExists } = await pg.query(`SELECT * FROM skytobi."Inventory" WHERE itemname = $1 AND branch = $2`, [itemname, br]);
            ...
            // Generate itemid
            const { rows: items } = await pg.query(`SELECT itemid FROM skytobi."Inventory" ORDER BY itemid DESC LIMIT 1`);
            ...
            for (let i = 0; i < departments.length; i++) {
                ...
                const { rows: deptExists } = await pg.query(`SELECT * FROM skytobi."Department" WHERE id = $1`, [dept]);
                ...
                await pg.query(`INSERT INTO skytobi."Inventory" (itemid, itemname, department, branch, units, cost, applyto, itemclass, composite, price, pricetwo, beginbalance, qty, minimumbalance, "group", description, imageone, imagetwo, imagethree, status, "reference", transactiondate, transactiondesc, reorderlevel, dateadded, createdby) 
