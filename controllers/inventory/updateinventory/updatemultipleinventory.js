...
const getFallbackData = async (itemid, department) => {
    ...
    const fallbackResult = await pg.query(
        `SELECT * FROM skytobi."Inventory" WHERE itemid = $1 AND department = $2 ORDER BY id DESC LIMIT 1`, 
        [itemid, department]
    );
    ...
    try {
        ...
        for (const [index, item] of items.entries()) {
            ...
            if (!department) {
                const { rows: itemDepartments } = await pg.query(
                    `SELECT department FROM skytobi."Inventory" WHERE itemid = $1 GROUP BY department`,
                    [itemid]
                );
            ...
            if (!branch) {
                for (let dept of departments) {
                    ...
                    const { rows } = await pg.query(
                        `SELECT branch FROM skytobi."Department" WHERE id = $1`,
                        [dept]
                    );
        ...
        for (const item of items) {
            ...
            const { rows: branchName } = await pg.query(
                `SELECT branch FROM skytobi."Branch" WHERE id = $1`,
                [branch]
            );
