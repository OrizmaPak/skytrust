...
const updateinventory = async (req, res) => {
    ...
    try {
        ...
        if (!department) {
            const { rows: itemDepartments } = await pg.query(`SELECT department FROM skytobi."Inventory" WHERE itemid = $1 GROUP BY department`, [itemid]);
            departments = [...new Set(itemDepartments.map(d => d.department))];
        }
        // Fetch the department with the highest id for the itemid
        let { rows: maxIdDepartment } = await pg.query(`SELECT * FROM skytobi."Inventory" WHERE itemid = $1 ORDER BY id DESC LIMIT 1`, [itemid]);

        if(department){
            maxIdDepartment = await pg.query(`SELECT * FROM skytobi."Inventory" WHERE itemid = $1 AND department = $2`, [itemid, department]);
        ...
        // if (!department) {
        //     const { rows: allDepartments } = await pg.query(`SELECT department FROM skytobi."Inventory" WHERE itemid = $1 GROUP BY department`, [itemid]);
        ...
        if(!branch)for (let dept of departments) {
            ...
            const { rows } = await pg.query(`SELECT branch FROM skytobi."Department" WHERE id = $1`, [dept]);
        ...
        for (let i = 0; i < departments.length; i++) {
            ...
            await pg.query(`INSERT INTO skytobi."Inventory" (itemid, branch, department, itemname, units, cost, price, pricetwo, beginbalance, qty, minimumbalance, "group", applyto, itemclass, composite, compositeid, description, imageone, imagetwo, imagethree, status, "reference", transactiondate, transactiondesc, dateadded, createdby) 
        ...
        // Log activity
        if (branch) {
            const { rows: branchName } = await pg.query(`SELECT branch FROM skytobi."Branch" WHERE id = $1`, [branch]);
