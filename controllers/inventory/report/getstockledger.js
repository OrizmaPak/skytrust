...
const getStockLedger = async (req, res) => {
    try { 
        ...
        const { rows: inventory } = await pg.query(`SELECT * FROM skytobi."Inventory" WHERE itemid = $1 AND transactiondate >= $2 AND transactiondate <= $3 AND status = 'ACTIVE' AND qty != 0 ${branch ? `AND branch = '${branch}'` : ''} ${department ? `AND department = '${department}'` : ''}`, [itemid, startdate, enddate]);

        // Fetch branch names for the inventory items
        const branchIds = inventory.map(item => item.branch);
        const { rows: branchNames } = await pg.query(`SELECT id, branch FROM skytobi."Branch" WHERE id = ANY($1::int[])`, [branchIds]);
        ...
        const { rows: departmentNames } = await pg.query(`SELECT id, department FROM skytobi."Department" WHERE id = ANY($1::int[])`, [departmentIds]);
